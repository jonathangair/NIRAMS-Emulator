...
def makeplots(col,whicherrcol,basename,showplots):
	...
	xpred = np.column_stack((x1v,x2v,x3v,x4v))
	#print(col)
	#print(xpred)
...
def fitOptGP(filebasename,showplots,nplots):
	...
	#results=op.minimize(nll,p0,method='TNC')
	#while True:
	#	try:
	#		results=op.minimize(nll,p0,method='TNC')
	#		#results=op.minimize(nll,p0,method='Powell')
	...
	#		#results=op.minimize(nll,p0,method='TNC')
	#		break
	#results=op.minimize(nll,p0,method='CG')
	results=op.minimize(nll,p0,method='BFGS')
	#results=op.minimize(nll,p0,method='L-BFGS-B')
	#results=op.minimize(nll,p0,method='TNC')
	#results=op.minimize(nll,p0,jac=grad_nll)
	gp.kernel.set_parameter_vector(results.x)
	print(np.exp(gp.kernel.get_parameter_vector()))
	gp.recompute()
...
def SamplePosterior(filebasename,showplots):
    ...
    sampler.run_mcmc(state, nproduction)
...
datafile=r'../../data/IndividualGWBGroupData_2020/GWBGroupData_YearAvsProc_%i' % (site)
calibfile=r'../data/NewData2020/GWBody6YrMeasuredAv.txt'
...
cellfile=r'../../data/IndividualGWBGroupData_2020/GWBGroupCellList_%i' % (site)
f=open(cellfile,"r")
...
kernel=100.0*kernels.ExpSquaredKernel([1.08,0.0015,0.00005,0.15],ndim=4)
#kernel=kernels.ExpSquaredKernel([0.03,0.01,0.0001,0.1],4)
#kernel=kernels.ExpSquaredKernel([10,10,10,10],4)
#kernel=kernels.ExpSquaredKernel(0.0001,4)
gp=george.GP(kernel)
...
g3.create_dataset('Kernel parameters',data=gp.kernel.get_parameter_vector())
